...
########################################################################
#######		restore save games to read-only wine-prefixes		 #######
########################################################################
...
function createPrefixDirs ()
{
	...
	do 
		if [ ! -d "$dir" ]
		then
			echo Created user wine prefix at $userWinePrefixes/$dir
			mkdir -p $userWinePrefixes/$dir
	done
}

function restoreGameSaves ()
{
	for (( i=0; i<${#userGameDirs[@]}; i++ ));
	do
		rsync -aP --dry-run "$userBackupDir"/"${userGameDirs[$i]}" "$userWinePrefixes"/"${userGameDirs[$i]}"
		printf '\n'
	done
}	# inverse of "backupGameSaves()" in savegame-backuper.sh

function restoreGameExceptions
{
	for (( i=0; i<${#userGameExceptions[@]}; i++ ));
	do
		rsync -aP --dry-run "$userBackupDir"/"${userGameExceptions[$i]}" "$userWinePrefixes"/"${userGameExceptions[$i]}"
		printf '\n'
	done
}	# inverse of "backupGameExceptions()" in savegame-backuper.sh
